...
var _Math = __webpack_require__(127);
...
var WMA = function () {
  ...
  _createClass(WMA, [{
    ...
    value: function match(obj) {
      ...
      this.keys.map(function (_ref) {
        var key = _ref.key,
            m = _ref.m;
        return _this._mapSource(key, m);
      });
      for (var id in this.dists) {
        this.results.push(this._setScoreObject(id));
      }this.results.sort(function (a, b) {
        return a.score - b.score;
      }).map(function (o, i) {
        if (i % _this.output === 0) _this.matchIndex++;
        var matchingPercent = (0, _Math.roundDecimal)(Math.abs((0, _Math.mapRange)(o.score, _this._min, _this._max, 0, 100) - 100), _this.decimals);

        _this._setObject(o, {
          matchIndex: _this.matchIndex,
          score: (0, _Math.mapRange)(o.score, _this._min, _this._max, 0, 100),
    ...
    key: '_mapSource',
    value: function _mapSource(key, m) {
      var _this2 = this;

      this.source.map(function (i) {
        if (!(key in i)) return;
        if (i.id === _this2.obj.id && !_this2.showOriginal) return;
        if (!_this2.dists[i.id]) _this2.dists[i.id] = [];

        _this2.dists[i.id].push((0, _Math.calcDist)(i[key], _this2.obj[key]) * m);
      });
    }
  }, {
    key: '_setScoreObject',
    value: function _setScoreObject(id) {
      var score = this._calculateScore(id);
      this._setmax(score);

      return {
        score: score,
        item: this.source.filter(function (i) {
          return i.id === id;
        })[0]
      };
    }
    ...
    key: '_calculateScore',
    value: function _calculateScore(id) {
      return this.dists[id].reduce(function (a, b) {
        return a + b;
      }, 0);
    }
  }, {
    key: '_setmax',
    value: function _setmax(score) {
      if (this._max < score || this._max === undefined) this._max = score;
    }
  }, {
    key: '_setObject',
    value: function _setObject(obj, keys) {
      for (var key in keys) {
        obj[key] = keys[key];
      }
    }
